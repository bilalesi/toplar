...
import { getProperties } from '@src/utils/apiUtils';
...
export const PropertiesListScreen: React.FC<Props> = React.memo(({ navigation }) => {
	...
	useFocusEffect(
		React.useCallback(() => {
			async function load() {
				try {
					const data = await getProperties(filtersState.available);
					setItems(data);
				} catch (error) {
