...
import { getLngDeltaFromLatDelta } from '@src/utils/mapUtils';
import { mapStyle } from './mapStyle';
import { getUserData } from '@src/utils/apiUtils';
...
export const PropertyScreen: React.FC<Props> = React.memo(({ route, navigation }) => {
	...
	React.useEffect(() => {
		const load = async () => {
			try {
				const data = await getUserData(item.data.creatorUserId);
				if (data) {
					setCreatorUser({ id: item.data.creatorUserId, data });
				}
			} catch (error) {
			...
			<ScrollView style={styles.scrollContainer} directionalLockEnabled>
				...
				{creatorUser && (
					<View style={styles.sectionTertiary}>
						<UserCard
							email={
								accountState.user?.id === creatorUser.id
