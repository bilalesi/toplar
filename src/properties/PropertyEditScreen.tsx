...
import { Button, TextInput, Switch, Text, Divider, IconButton } from 'react-native-paper';
...
export const PropertyEditScreen: React.FC<Props> = React.memo(({ route, navigation }) => {
	...
	const canSubmit =
		...
		pcmGbp.trim().length &&
		Number(pcmGbp.trim()) > 0 &&
		sizeSqm.trim().length &&
		Number(sizeSqm.trim()) > 0 &&
		numRooms.trim().length &&
		Number(numRooms.trim()) > 0 &&
		...
		const submit = async () => {
			...
			try {
				...
				if (route.params?.item?.id) {
					await setPropertyData(route.params.item.id, data);
				} else {
					await addPropertyData(data, accountState.authUser!.uid);
				}
				navigation.popToTop();
			} catch (error) {
	...
	const handleDeletePress = React.useCallback(() => {
		const submit = async () => {
			...
			try {
				...
				await deletePropertyData(route.params.item.id);
				navigation.popToTop();
			} catch (error) {
				Alert.alert(error.message);
				setIsSubmitting(false);
		...
		<View style={styles.container}>
			...
			>
				...
				>
					...
					<Switch color="#6100ee" value={available} onValueChange={setAvailable} />
				</View>
